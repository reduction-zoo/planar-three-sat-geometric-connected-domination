...
Round 041 added two nontrivial negative targets outside the prepared corpus. A target-only checker verified the full integer unit graph's Clark path/side interface, contracted it to a 612-vertex skeleton, and used the published CDS-to-connected-cover implication. For each target, 240 mandatory articulation vertices plus 57 exact degree-two cover folds and an exhaustive 18-vertex residual optimum of 12 give lower bound 309 against connected-cover budget 308. This is a structural NO certificate conditional on the published lemma, rather than a generic direct CDS solver. The graph-atlas fold check and a positive geometric control limit the risk of an overstrong reduction rule; they do not replace expert review.
